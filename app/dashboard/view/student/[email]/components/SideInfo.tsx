...
const SideInfo = (props: any) => {
  return (
    <div className={`${!props.isDesktop ? 'w-1/4 ' : 'w-full '} px-1`}>
      ...
      <div className="font-extrabold text-blue-600 my-2  py-2 text-center rounded-lg shadow-2xl">{props.selectViewUser?.studentInformation?.Track}</div>
      ...
      <div className="text-center mt-2 font-extrabold">
        ...
        <p className="text-blue-600">{props.selectViewUser?.studentInformation?.memberQuota ?? 'USING GLOBAL'}</p>
      </div>

      <div className="text-center mt-2 font-extrabold">
        <p>Title Quota</p>
        <p className="text-blue-600">{props.selectViewUser?.studentInformation?.titleQuota ?? 'USING GLOBAL'}</p>
      </div>

      <div className="text-center mt-2 font-extrabold">
        <p>Supervisor</p>
        <p className="text-blue-600">{props.selectViewUser?.studentInformation?.LecturerInformation?.User?.name ?? ''}</p>
