...
export const useUpdateMobile = create<StudentStore>((set) => ({
  data: {},
  loading: false,
  ...
  sendData: async (postData) => {
    try {
      ...
      const response = await fetch('/api/v1/userProfile/updateMobile', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        toast.success('Sucessfully update mobile', {
          position: 'top-right',
          autoClose: 5000,
        ...
        toast.error('Failed update mobile', {
          position: 'top-right',
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
