...
export const useUpdateEmail = create<StudentStore>((set) => ({
  data: {},
  loading: false,
  ...
  sendData: async (postData) => {
    try {
      ...
      const response = await fetch('/api/v1/userProfile/updateEmail', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        toast.success('Sucessfully update email', {
          position: 'top-right',
          autoClose: 5000,
        ...
        toast.error('Failed add  update email', {
          position: 'top-right',
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
