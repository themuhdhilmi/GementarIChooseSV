          ...
          <div className={`flex ${!isDesktop ? 'flex-row' : 'flex-col'} flex-wrap `}>
            <div className={`${!isDesktop ? 'w-1/4 ' : 'w-full '} px-1`}>
              <div className="flex flex-row bg-red-600 rounded-lg shadow-lg">
                ...
                <div className="my-3 w-full ">
                  ...
                  <p className="text-sm text-slate-100">{props.selectViewUser?.lecturerInformation?.matricNumber}</p>
            ...
            <div className={`${!isDesktop ? 'w-3/4 ' : 'w-full '}  px-4 `}>
              <div className="overflow-x-auto px-5 rounded-lg shadow-lg">
                ...
                {props.selectViewUser?.lecturerInformation?.Member?.length === 0 ? (
                  <div className="py-6">
                    <HereIsEmpty />
                  </div>
                ) : (
                  <table className="table ">
                    <tbody>
                      {props.selectViewUser?.lecturerInformation?.Member.map((item: any, index: number) => {
                        return (
                          <tr key={index}>
                      ...
                      {props.selectViewUser?.lecturerInformation?.Member.map((item: any, index: number) => {
                        return (
                          <tr key={index}>
                            ...
                            <td>
                              {' '}
                              <Link href={'/test'} className="text-blue-600 text-sm">
                                Poster 1 (X)
                              </Link>
                            </td>
                          </tr>
