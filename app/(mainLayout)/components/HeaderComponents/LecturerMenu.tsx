    ...
    <div>
      ...
      {!isDesktop ? (
        <div className="flex flex-row gap-3 py-5">
          ...
          <Link href={`/dashboard/view/lecturer/${props.email}`} className={`btn btn-sm rounded-lg border-0  bg-red-800 text-white hover:bg-red-900 ${(getFirstThreePathName() + `/${decodeURIComponent(params.email)}`) === ('/dashboard/lecturer/lecturerManage'  + `/${props.email}`) ? '' : 'bg-opacity-0'} `}>
            <AiOutlineGlobal />
